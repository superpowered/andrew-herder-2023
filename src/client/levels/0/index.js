...
class Level {
    ...
    pixelSizes.forEach((pixelSize) => {
      this.game.textPixels = [
        ...
        ...pixelSize.pixels
          ...
          .map(
            (pixel) =>
              new TextPixel(pixel, pixelSize.size, pixelSize.absorption),
          )
  ...
  createIntroTextPixels(context) {
    ...
    context.fillText(
      introText,
      this.game.width / 2 / this.game.dpr,
      this.game.height / 2 / this.game.dpr,
    );
    ...
    context.fillText(
      'Senior Software Engineer | Milwaukee, WI',
      this.game.width / 2 / this.game.dpr,
      this.game.height / 2 / this.game.dpr + 30,
    );
    ...
    const pixels = Array.from(
      context.getImageData(0, 0, this.game.width, this.game.height).data,
    );
    ...
    for (let i = 0; i < pixels.length; i += 4) {
      ...
      pixelSizes.find((pixelSize, index) => {
        ...
        const neighbors = this.findMyNeigbors(
          i,
          pixelSize.size * pixelSize.size,
          pixels,
        );
